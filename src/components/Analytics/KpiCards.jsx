...
export default function KpiCards({ analytics = {} }) {
  const cards = [
    ...
    },
    {
      ...
      title: "Meeting Rate",
      value: analytics.meetingRate ?? "-",
      subtext: analytics.meetingRateSubtext ?? "vs last month",
    },
    {
      id: "avgDealSize",
      title: "Avg Deal Size",
      value: analytics.avgDealSize ?? "-",
      subtext: "ACV target: $10–15K",
    },
    {
      id: "signalToMeeting",
      title: "Signal → Meeting",
      value: analytics.signalToMeeting ?? "-",
      subtext: "Avg time to book",
    },
  ];
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
      {cards.map((card) => (
        ...
        >
          ...
          <p className="text-sm mt-3 text-gray-400">{card.subtext}</p>
        </div>
      ))}
    </div>
  );
