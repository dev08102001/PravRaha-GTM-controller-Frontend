...
import SignalFeedListItem from "../components/signals/SignalFeedListItem";
...
export default function Signals() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-4xl font-bold text-white">Signal Feed</h1>
        <p className="text-gray-400 mt-2">
          {query
            ? `Accounts matching “${query}” — synced with Top Target Companies`
            : "High-intent accounts from your knowledge base"}
        </p>
      </div>
      ...
      {items.length === 0 ? (
        <div className="bg-[#151D2E] rounded-2xl p-6 text-center text-gray-400">
          {query
            ? `No signals match “${query}”. Try a broader term like “software”, “fintech”, or “cybersecurity”.`
            : "Enter a campaign goal on the dashboard to filter signals."}
        </div>
      ) : (
        <div className="space-y-3">
          <p className="text-xs uppercase tracking-wider text-gray-500 font-semibold px-1">
